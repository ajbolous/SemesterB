...
public:
	...
	DiGraph(int n, double infinity):N(n) {
...
void print(DiGraph ad)
{
	...
	for (i = 0; i < n; i++)
	{
		for (j = 0; j < n; j++)
			...
			{
				sprintf_s(str, "%8.2lf", ad.getArcDistance(i, j));
				cout << str;
			} // else
		cout << endl;
	} // for  
...
public:
	...
	ADiGraph(int n, double inf) : DiGraph(n, inf) {
	...
	void shortest_distances(double* distances) {
		DiGraph d(N, infinity);
		distances[0] = 0;
